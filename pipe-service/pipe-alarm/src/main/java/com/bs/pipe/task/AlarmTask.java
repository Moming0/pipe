...
public class AlarmTask {
    ...
    public void piezometerAlarmTask(){
        ...
        try {
            List<PiezometerVO> pList = piezometerService.selectPressureLogAvgSearch(new Piezometer() , startTime, endTime, "0");
                ...
                for (RtaAlarmStandard alarmStandard : alarmStandardList) {
                    if(alarmStandard.getRegionId().equals(piezometerVO.getRegionId())){
                        try {
                            ...
                            Double logDif = ArithmeticUtil.mul(ArithmeticUtil.sub(piezometerVO.getElevation(), alarmStandard.getElevation()), 0.01);
        ...
        try {
            List<BuildingVO> bList = buildingService.selectBuildingAndPressure(new Building(), startTime, endTime, "0", "0");
            for (BuildingVO buildingVO : bList) {
                ...
                for (RtaAlarmStandard alarmStandard : alarmStandardList) {
                    if(alarmStandard.getRegionId().equals(buildingVO.getRegionId())){
                        try {
                            //根据高程差计算压力 1m = 0.01MPa
                            Double logDif = ArithmeticUtil.mul(ArithmeticUtil.sub(buildingVO.getElevation(), alarmStandard.getElevation()), 0.01);
                            Double minScale = ArithmeticUtil.add(alarmStandard.getMinScale(), (logDif > 0 ? logDif * -1 : logDif < 0 ? logDif * -1 : 0));
                            Double maxScale = ArithmeticUtil.add(alarmStandard.getMaxScale(), (logDif > 0 ? logDif * -1 : logDif < 0 ? logDif * -1 : 0));
        ...
        try {
            List<BuildingVO> bList = buildingService.selectBuildingAndPressure(new Building(), startTime, endTime, "0", "1");
            for (BuildingVO buildingVO : bList) {
                boolean flag = false;
                List<LinkedHashMap<String,Object>> list = new ArrayList<LinkedHashMap<String,Object>>();
                ...
                for (RtaAlarmStandard alarmStandard : alarmStandardList) {
                    if(alarmStandard.getRegionId().equals(buildingVO.getRegionId())){
                        try {
                            //根据高程差计算压力 1m = 0.01MPa
                            Double logDif = ArithmeticUtil.mul(ArithmeticUtil.sub(ArithmeticUtil.add(buildingVO.getElevation(),buildingVO.getBuildingHeight()), alarmStandard.getElevation()), 0.01);
                            Double minScale = ArithmeticUtil.add(alarmStandard.getMinScale(), (logDif > 0 ? logDif * -1 : logDif < 0 ? logDif * -1 : 0));
                            Double maxScale = ArithmeticUtil.add(alarmStandard.getMaxScale(), (logDif > 0 ? logDif * -1 : logDif < 0 ? logDif * -1 : 0));
                            for (LogPressure logP : buildingVO.getLogPressure()) {
                                if(alarmStandard.getCategory().equals("" + logP.getReadTime().getHours())){
